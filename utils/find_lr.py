...
class LRFinder(object):
    ...
    def range_test(
        ...
        val_loader=None,
        end_lr=100,
        num_iter=100, # bump this up to 100 later
        step_mode="exp",
        smooth_f=0.05,
        diverge_th=5,
    ):
    ...
    def plot(self, skip_start=10, skip_end=5, log_lr=True, out_dir=None):
        ...
        if out_dir is not None:
            plt.savefig(os.path.join(out_dir, 'lr_finder.png'))
        plt.show()
...
def parse_args():
    ...
    parser.add_argument('--step_mode', dest='step_mode',
                        help='configuration filename',
...
def find_lr(model, criterion, optimizer, train_loader, val_loader=None):


    lr_finder = LRFinder(model, optimizer, criterion, device='cuda')
    lr_finder.range_test(train_loader, val_loader, end_lr=0.1, num_iter=50, step_mode=args.step_mode)
    log_lr = args.step_mode == 'exp'
    lr_finder.plot(log_lr=log_lr, out_dir=None)
