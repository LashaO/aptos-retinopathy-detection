...
from utils.preprocess import crop_image_from_gray
...
def test():
    transform = transforms.Compose([transforms.CenterCrop(70),
                                    transforms.ToTensor()])

    dataset = SmallDataset('data', 'train', transform)
    print(len(dataset))
    example = dataset[0]
    example = dataset[1]
    print(example)

    dataset = SmallDataset('data', 'val', transform)
    print(len(dataset))
